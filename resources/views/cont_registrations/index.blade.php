<x-app-layout>
    ...
    <script>
        var registrationsData = "{{ route('registrations.data') }}" + "?defer=0";
        ...
        $(document).ready(function() {
                ...
                , columns: [{
                        ...
                        , {
                            ...
                            , render: function(data, type, row) {
                                ...
                                if (row.defer_status < 3 && row.approval_status !== 1) {
                                    buttons += `
                                    <span class="defer-btn badge bg-danger" data-id="${row.id}">
                                        DEFER (${row.defer_status})
                                    </span>
                                ...
                                <div class="action-btns">
                                    <span class="view-btn badge bg-secondary" data-id="${row.id}">
                                        VIEW
                                    </span>
                                    ${buttons}
                                </div>
                ...
                , fnDrawCallback() {
                    ...
                    $(".defer-btn").on("click", function() {
                        ...
                        if (confirm("Are you sure you want to defer?")) {
                            $.ajax({
                                url: "{{ route('registrations.defer', ':id') }}".replace(':id', registrationId)
                                , type: "PATCH"
                                , headers: {
                    ...
                    $(".approve-btn").on("click", function() {
                        ...
                        if (confirm("Are you sure you want to approve?")) {
                            $.ajax({
                                url: "{{ route('registrations.approve', ':id') }}".replace(':id', registrationId)
                                , type: "PATCH"
                                , headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                }
            ...
            function updateDataTableURL(deferValue, approvalValue) {
                ...
                registrationsData = "{{ route('registrations.data') }}" + queryParams;
