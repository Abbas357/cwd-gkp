...
<div class="row standardization-details">
    <div class="col-md-12">
        ...
        <table class="table table-bordered">
            @foreach(['owner_name', 'district', 'address', 'cnic', 'email'] as $field)
            <tr>
                ...
                <td class="d-flex justify-content-between align-items-center gap-3" style="padding: 0.1rem 0.5rem; vertical-align:middle">
                    <span id="text-{{ $field }}">{{ $mobile_number->$field }}</span>
                    <input type="text" id="input-{{ $field }}" value="{{ $mobile_number->$field }}" class="d-none form-control" />
                    <button id="save-btn-{{ $field }}" class="btn btn-sm btn-primary d-none" onclick="updateField('{{ $field }}', {{ $mobile_number->id }})">Update</button>
            ...
            <div class="col-md-3 mb-3">
                <div class="card">
                    <label for="{{ $imageField }}">
                        <img src="{{ $mobile_number->hasMedia($imageField) ? $mobile_number->getFirstMediaUrl($imageField) : 'https://via.placeholder.com/150x247' }}" id="{{ $imageField }}-prev" class="card-img-top" alt="{{ $label }}">
                        <div class="overlay">
                            <span>{{ $mobile_number->hasMedia($imageField) ? 'Change Image' : 'Add Image' }}</span>
                        </div>
                    </label>
...
<script>
    ...
    $('#print-standardization').on('click', () => {
        $(".standardization-details").printThis({
            pageTitle: "Standardization details of {{ $mobile_number->product_name }}"
        });
    });
