<x-dtms-layout title="High Cost Districts Report">
    @push('style')
    ...
    <x-slot name="header">
        ...
        <li class="breadcrumb-item active" aria-current="page">High Cost Districts</li>
    </x-slot>
    ...
    <div class="container py-2 px-1 rounded">
        <div class="report-header">
            <div class="report-title">
                ...
                <div class="report-metadata">
                    ...
                    <span class="badge bg-info">Cost Type: {{ ucfirst($costType) }}</span>
                </div>
            </div>
        </div>
                ...
                <form method="get" class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="cost_type" id="cost_type" class="form-control">
                            <option value="total" {{ request()->query('cost_type') == 'total' ? 'selected' : '' }}>Total Cost</option>
                            <option value="restoration" {{ request()->query('cost_type') == 'restoration' ? 'selected' : '' }}>Restoration Cost</option>
                            <option value="rehabilitation" {{ request()->query('cost_type') == 'rehabilitation' ? 'selected' : '' }}>Rehabilitation Cost</option>
                        </select>
                    </div>
                    <div class="d-flex justify-content-between p-3">
                        <div>
                            ...
                            <a href="{{ route('admin.apps.dtms.reports.highly-damaged') }}" class="cw-btn light ms-3">
        ...
        <div class="table-responsive">
            <table id="high-cost-districts-report" class="table table-bordered table-hover">
                <thead>
                ...
                <tbody>
                    ...
                    <tr class="align-middle">
                        ...
                        <td class="text-center {{ $costType == 'restoration' ? 'cost-highlight' : '' }}">
                            {{ number_format($stats['restoration_cost'], 2) }}
                        </td>
                        <td class="text-center {{ $costType == 'rehabilitation' ? 'cost-highlight' : '' }}">
                            {{ number_format($stats['rehabilitation_cost'], 2) }}
                        </td>
                        <td class="text-center {{ $costType == 'total' ? 'cost-highlight' : '' }} fw-bold">
                    ...
                    <tr class="fw-bold total-row">
                        ...
                        <td class="text-center {{ $costType == 'restoration' ? 'cost-highlight' : '' }}">
                            {{ number_format($total['total_restoration_cost'], 2) }}
                        </td>
                        <td class="text-center {{ $costType == 'rehabilitation' ? 'cost-highlight' : '' }}">
                            {{ number_format($total['total_rehabilitation_cost'], 2) }}
                        </td>
                        <td class="text-center {{ $costType == 'total' ? 'cost-highlight' : '' }}">
                            {{ number_format($total['total_cost'], 2) }}
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <script>
        $(document).ready(function() {
            $('#print-report').on('click', () => {
                $("#high-cost-districts-report").printThis({
                    header: "<h4 class='text-center mb-3'>High Cost Districts Report</h4>",
                    beforePrint() {
