<x-module-layout title="Vehicles">
    @push('style')
    ...
    <script>
        $(document).ready(function() {
            var table = initDataTable('#vehicles-datatable', {
                ajaxUrl: "{{ route('module.vehicles.index') }}"
                , columns: [{
                        data: "id"
                ...
                , customButton: {
                    ...
                    , action: function(e, dt, node, config) {
                        ...
                        pushStateModal({
                            fetchUrl: "{{ route('module.vehicles.create') }}"
                            , btnSelector: '.create-btn'
                            , title: 'Add Vehicle'
                            , actionButtonName: 'Add Vehicle'
                            , modalSize: 'lg'
                            , includeForm: true
                            , formAction: "{{ route('module.vehicles.store') }}"
                            , modalHeight: '50vh'
                            , hash: false
                       , }).then((modal) => {
            ...
            $("#vehicles-datatable").on('click', '.delete-btn', async function() {
                ...
                const url = "{{ route('module.vehicles.destroy', ':id') }}".replace(':id', vehicleId);
            ...
            pushStateModal({
                fetchUrl: "{{ route('module.vehicles.detail', ':id') }}",
            ...
            pushStateModal({
                fetchUrl: "{{ route('module.vehicles.history', ':id') }}",
            ...
            pushStateModal({
                fetchUrl: "{{ route('module.vehicles.allotment.create', ':id') }}"
                , btnSelector: '.allot-btn'
                , title: 'Vehicle Allotment'
                , actionButtonName: 'Allot Vehicle'
                , modalSize: 'lg'
                , includeForm: true
                , formAction: "{{ route('module.vehicles.allotment.store', ':id') }}"
                , modalHeight: '65vh'
            , }).then((modal) => {
