        ...
        <table class="table table-striped table-bordered" style="box-shadow: 0 0 15px #00000033">
            <tbody>
                <tr>
                    ...
                    <td>{{ $asset->type }}</td>
                    <th>Year</th>
                    <td>{{ $asset->model_year }}</td>
                </tr>
                <tr>
                    <th>Reg Number</th>
                    <td>{{ $asset->registration_number }}</td>
                    <th>Functional Status</th>
                    <td>{{ $asset->functional_status }}</td>
                </tr>
                <tr>
                    <th>Model</th>
                    <td>{{ $asset->model }}</td>
                    <th>Fuel Type</th>
                    <td>{{ $asset->fuel_type }}</td>
                </tr>
                @if($asset->allotment)
                    <tr>
                        <th>Alloted to</th>
                        <td colspan="3">
                            @if($asset->allotment->type === 'Pool')
                                @if($asset->allotment->office_id)
                                ...
                                @endif
                            @elseif($asset->allotment->type === 'Permanent' || $asset->allotment->type === 'Temporary')
                                @if($asset->allotment->user_id)
                                    <div class="d-flex justify-content-between">
                                        <span class="badge bg-primary fs-6">{{ $asset->allotment->type }}</span>
                                    ...
                                    <table class="table table-sm mt-2 mb-0 border">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{{ $asset->allotment->user->name ?? 'No Name' }}</td>
                                            </tr>
                                            <tr>
                                                <th class="bg-light">Designation</th>
                                                <td>{{ $asset->allotment->user->currentPosting?->designation?->name ?? 'Designation Missing' }}</td>
                                            </tr>
                                            <tr>
                                                <th class="bg-light">Office</th>
                                                <td>{{ $asset->allotment->user->currentPosting?->office?->name ?? 'Office Missing' }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                @elseif($asset->allotment->office_id)
                                    ...
                                    <table class="table table-sm mt-2 mb-0 border">
                                        <tbody>
                                            <tr>
                                                <th class="bg-light">Office</th>
                                                <td>{{ $asset->allotment->office->name ?? 'Office Missing' }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                @else
                                ...
                                @endif
                            @else
                                <span class="badge bg-secondary fs-6">{{ $asset->allotment->type ?? 'Unknown Type' }}</span>
                            @endif
                        </td>
                    </tr>
                @endif
