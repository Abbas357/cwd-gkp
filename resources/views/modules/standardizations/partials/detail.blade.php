...
<script>
    $(document).ready(function() {
        ...
        imageCropper({
            ...
            , onComplete: async function(file, input) {
                ...
                const url = "{{ route('module.standardizations.uploadFile', ':id') }}".replace(':id', '{{ $Standardization->id }}');
                try {
    ...
    async function updateField(field, id) {
        ...
        const url = "{{ route('module.standardizations.updateField', ':id') }}".replace(':id', id);
        const data = {
            field: field
            , value: newValue
        };
