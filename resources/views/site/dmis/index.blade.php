<x-main-layout title="District Wise Damages {{ setting('activity', 'dmis') }} {{ setting('session', 'dmis') }}">
    ...
    <x-slot name="breadcrumbTitle">District Wise Damages {{ setting('activity', 'dmis') }} ({{ setting('session', 'dmis') }})</x-slot>
                ...
                <form method="GET" action="{{ route($activity . '.index') }}" class="row g-3" id="filterForm">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="duration" id="duration" class="form-control">
                            <option value="90" {{ request()->query('duration') == '90' ? 'selected' : '' }}>Last 90 days</option>
                            <option value="45" {{ request()->query('duration') == '45' ? 'selected' : '' }}>Last 45 days</option>
                            <option value="30" {{ request()->query('duration') == '30' || !request()->query('duration') ? 'selected' : '' }}>Last 30 days</option>
                            <option value="15" {{ request()->query('duration') == '15' ? 'selected' : '' }}>Last 15 days</option>
                            <option value="Custom" {{ request()->query('duration') == 'Custom' ? 'selected' : '' }}>Custom</option>
                        </select>
                    </div>
        ...
        <div class="table-responsive">
            <div class="table-container" id="generated-report" >
                <div class="text-center mb-2">
                    <h5 class="mb-0">
                        District Wise <span class="px-2 py-1 bg-light shadow-sm rounded border text-dark">{{ $type ?? "Road" }}s</span> Damage Report
                    </h5>
                    @if(isset($startDate) && isset($endDate))
                    @if($startDate->format('Y-m-d') === $endDate->format('Y-m-d'))
                    <div class="text-dark mt-1">Date: {{ $startDate->format('F d, Y (l)') }}</div>
                    @else
                    <div class="text-dark mt-1">From {{ $startDate->format('F d, Y') }} to {{ $endDate->format('F d, Y') }}</div>
                    @endif
                    @else
                        ...
                        <tr class="align-middle">
                            ...
                            <td class="text-center no-print">
                                <div class="action-buttons">
                                    <a href="{{ route($activity . '.detail.district', ['district' => $stats['district']->name]) }}?type={{ $type }}" class="btn btn-white border btn-detail" title="View Details">
