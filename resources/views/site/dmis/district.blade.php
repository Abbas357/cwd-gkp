<x-main-layout title="District Damage Details - {{ $district->name }}">
    <x-slot name="breadcrumbTitle">{{ $district->name }} Detail {{ setting('activity', 'dmis') }} ({{ setting('session', 'dmis') }})</x-slot>
    ...
    <div class="container">
        <div id="district-report" class="py-2 px-1">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="no-print">
                    <button type="button" id="print-report" class="cw-btn primary">
                ...
                <table class="damage-table">
                    ...
                    <tbody>
                        ...
                        <tr class="infrastructure-row">
                            <td rowspan="{{ $totalDamages + ($infrastructureDamages->where('remarks', '!=', '')->count() > 0 ? $infrastructureDamages->where('remarks', '!=', '')->count() : 0) }}">
                                {{ $counter++ }}
                            </td>
                            <td rowspan="{{ $totalDamages + ($infrastructureDamages->where('remarks', '!=', '')->count() > 0 ? $infrastructureDamages->where('remarks', '!=', '')->count() : 0) }}">
                                <strong>{{ $infrastructure->name }} </strong> <span style="font-size:10px"> {{ $infrastructureDamages->count() > 1 ? "({$infrastructureDamages->count()} Damages)" : '' }}</span>
                            </td>
                            <td rowspan="{{ $totalDamages + ($infrastructureDamages->where('remarks', '!=', '')->count() > 0 ? $infrastructureDamages->where('remarks', '!=', '')->count() : 0) }}">
                                {{ $infrastructure->type }}
                            </td>
                        ...
                        <tr class="remarks-row">
                            <td colspan="5">
                                <strong>Remarks:</strong> {{ $firstDamage->remarks }}
                            </td>
                        </tr>
                        @endif
                        ...
                        <tr class="remarks-row">
                            <td colspan="5">
                                <strong>Remarks:</strong> {{ $damage->remarks }}
                            </td>
                        </tr>
                        @endif
                        @endforeach
