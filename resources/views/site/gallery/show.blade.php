<x-main-layout title="{{ $galleryData['title'] }}">
    ...
    <div class="container mt-4">
        <h3 class="mb-4">Comments</h3>
            @foreach ($galleryData['comments'] as $comment)
                <div class="card mb-3 shadow-sm">
                    <div class="card-body">
                        ...
                        <div id="replyForm-{{ $comment->id }}" class="collapse mt-3">
                            <form class="needs-validation" method="POST" action="{{ route('comments.store', ['type' => 'Gallery', 'id' => $galleryData['id']]) }}" novalidate>
                                @csrf
        ...
        <!-- Main Comment Form -->
        <form class="container needs-validation p-2" method="POST" action="{{ route('comments.store', ['type' => 'Gallery', 'id' => $galleryData['id']]) }}" novalidate>
            @csrf
            <div class="mb-3 d-flex justify-content-between gap-3 inputs d-none">
                <input type="text" name="name" class="form-control" placeholder="Your Name" required style="flex: 1;" />
                <input type="email" name="email" class="form-control" placeholder="Your Email" required style="flex: 1;" />
